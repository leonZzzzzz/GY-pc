    ...
    <el-form label-position="right" label-width="180px" style="margin:40px 100px;">
        <el-form-item :label="minConfig.name" :rules="[{required:true,message:'该项为必填项'}]">
          <el-input v-model="minConfig.value" style="width:140px">
            ...
            <el-popover width="400" style="color:red!important" placement="top-start" trigger="hover" content="买家单次提现最小金额，低于这个金额，买家将不能提交提现申请">
        ...
        <el-form-item :label="maxConfig.name" :rules="[{required:true,message:'该项为必填项'}]">
          <el-input v-model="maxConfig.value" style="width:140px">
            ...
            <el-popover width="400" style="color:red!important" placement="top-start" trigger="hover" content="买家单次提现最高金额，高于这个金额，买家将不能提交提现申请">
                <el-button class="msg-popu-error" icon="el-icon-warning-outline" slot="reference"></el-button>
            </el-popover>
        </el-form-item>
        <el-form-item :label="timesConfig.name" :rules="[{required:true,message:'该项为必填项'}]">
          <el-input v-model="timesConfig.value" style="width:140px">
            ...
            <el-popover width="400" style="color:red!important" placement="top-start" trigger="hover" content="买家每日可提现次数，高于这个次数，买家将不能提交提现申请">
                <el-button class="msg-popu-error" icon="el-icon-warning-outline" slot="reference"></el-button>
            </el-popover>
        </el-form-item>
       <el-form-item :label="configItem.name" :rules="[{required:true,message:'该项为必填项'}]">
          <el-input v-model="configItem.value" style="width:140px">
            ...
            <el-popover width="400" style="color:red!important" placement="top-start" trigger="hover" content="买家提现手续费:比如提现手续费为0.3%。买家提现1000元时，则产生3元手续费，实际收到997元。">
                <el-button class="msg-popu-error" icon="el-icon-warning-outline" slot="reference"></el-button>
            </el-popover>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getConfigItem() {
      this.$http.getConfigByKey('STORE_POUNDAGE_MEMBER').then(res => {
        this.configItem = res.data.data
      })
    },
    getMinConfig(){
        this.$http.getConfigByKey('MIN_STORE_WITHDRAW_MEMBER').then(res => {
          this.minConfig = res.data.data
          this.minConfig.value=this.minConfig.value/100
        })
    },
    getMaxConfig(){
        this.$http.getConfigByKey('MAX_STORE_WITHDRAW_MEMBER').then(res => {
          this.maxConfig = res.data.data
          this.maxConfig.value=this.maxConfig.value/100
        })
    },
    getTimesConfig(){
        this.$http.getConfigByKey('TIMES_STORE_WITHDRAW_MEMBER').then(res => {
        this.timesConfig = res.data.data
        })
    },
    getAuditTypeConfig(){
        this.$http.getConfigByKey('AUDIT_TYPE_MEMBER').then(res => {
        this.auditType = res.data.data
        })
    },
    getAuditTimeConfig(){
        this.$http.getConfigByKey('AUTO_AUDIT_TIME_MEMBER').then(res => {
