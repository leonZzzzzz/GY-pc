    ...
    <el-form :model="model" ref="productModel" label-position="left" label-width="100px">
      <QcTitle title="基本信息">
        <el-form-item label="店铺分类">
          <el-select
            v-model="model.storeCategoryName"
            style="width:450px;"
            @change="currOperatorChange"
          >
            <el-option :label="item.name" :value="item.id" v-for="(item, i) in storeLists" :key="i"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item
          label="商品名称"
        ...
        >
          ...
          <p
            style="font-size: 12px;color: #888;padding: 10px 0;line-height: 3;"
          >建议尺寸：750*750，最大不超过3M</p>
        </el-form-item>
      ...
      <QcTitle title="库存信息">
        <el-form-item label="商品规格">
          <div class="property-box">
            ...
            <el-button style="width: 150px;" @click="addProperty()" v-if="property.length < 1">添加规格</el-button>
            <p
              style="font-size: 12px;color: #ff8f76;"
            >*如果商品属于多规格现价、库存将不可填写。现价、默认获取规格中现价最低值。库存默认所有库存总和</p>
          </div>
          <table class="property-table" v-if="property.length" style="overflow: auto;">
            <tbody>
              <tr>
                <th v-for="(item, i) in tableTitle" :key="i">
                  <div class="input-group" v-if="item.inputType">
                    <el-select
                      v-if="item.inputType == 'unit'"
                      v-model="item.value"
                      @change="unitSelectChange($event, i)"
                      style="width:100px"
                      placeholder="请选择"
                    >
                      <el-option
                        v-for="itemUnit in unitList"
                        :key="itemUnit"
                        :label="itemUnit"
                        :value="itemUnit"
                      ></el-option>
              ...
              <tr v-for="(item, i) in tableBody" :key="i">
                <template v-for="(label, index) in item.itemValue">
                  <td
                    ...
                    class="border-left"
                  >
                    <div v-if="label.inputType">
                      <el-select
                        v-if="label.inputType == 'unit'"
                        v-model="label.value"
                        @change="unitSelectChange2($event, i)"
                        style="width:100px"
                        placeholder="请选择"
                      >
                        <el-option
                      ...
                      <el-input
                        ...
                        :style="`width:140px`"
                        v-model="label.value"
                        :disabled="label.disabled ? true : false"
        ...
        <el-form-item label="单位" prop="unit" v-else>
          <el-select
            :disabled="model.isMultiSpec"
            v-model="model.unit"
            style="width:220px"
        ...
        <el-form-item label="现价(元)" prop="price" :rules="[{ required: true, message: '该字段为必填项' }]">
        ...
        <el-form-item
          label="重量(kg)"
          prop="weight"
          :rules="[{ required: model.unit == '斤' ? false : true, message: '该字段为必填项' }]"
        >
          <el-input
            v-if="model.unit == '斤'"
            :disabled="true"
        ...
        <el-form-item label="库存(件)" prop="qty" :rules="[{ required: true, message: '该字段为必填项' }]">
...
export default {
  data() {
    return {
      ...
      tableTitle: [
        // 规格参数 要在这里添加
        { value: '', title: '原价(元)', inputType: 'origPrice' },
        { value: '', title: '现价(元)', inputType: 'price' },
        // { value: '', title: '会员价', inputType: 'price' },
        { value: '', title: '成本价(元)', inputType: 'price' },
        { value: '', title: '重量(kg)', inputType: 'number', prop: 'weight', disabled: true },
        { value: '', title: '最低起订量(件)', inputType: 'number' },
        { value: '', title: '库存(件)', inputType: 'number' },
        { value: '', title: '单位(如: 斤)', inputType: 'unit', prop: 'unit' },
      ],
      tableBody: [
        {
          itemValue: [
            // 规格参数 要在这里添加
            { value: '', placeholder: '原价(元)', inputType: 'origPrice' },
            { value: '', placeholder: '现价(元)', inputType: 'price' },
            // { value: '', placeholder: '会员价', inputType: 'price' },
            { value: '', placeholder: '成本价(元)', inputType: 'price' },
            { value: '0.5', placeholder: '重量(kg)', inputType: 'number', prop: 'weight', disabled: true },
            { value: '1', placeholder: '最低起订量(件)', inputType: 'number' },
            { value: '', placeholder: '库存(件)', inputType: 'number' },
            { value: '斤', placeholder: '单位(如:斤)', inputType: 'unit'},
          ]
        }
      ],
      supplierLists: [],
      ...
      brandLists: [],
      levelList: [],
      unitList: ['斤', '件', '袋', '箱', '瓶', '包', '个', '盒', '罐','筐','双','捆','把','提','份','板','桶'],
      showUnit:true
    }
  },
  watch: {
  ...
  methods: {
    // 获取店铺分类
    getList(){
      this.$http.getAllCate().then(res => {
        this.storeLists = res.data.data
      })
    },
    currOperatorChange(e){
      console.log(e)
      this.storeLists.map(item=>{
        if(e==item.id){
          this.model.storeCategoryId=item.id
          this.model.storeCategoryName=item.name
        }
      })
    },
    ...
    compareValue() {
      ...
      this.model.minOrderQuantity = Number(sepc[0].minOrderQuantity)
      this.model.unit = sepc[0].unit
      if (this.model.unit == '斤') {
    ...
    getItemValue() {
      return [
        ...
        { value: '1', placeholder: '最低起订量(件)', inputType: 'number'},
        { value: '', placeholder: '库存(件)', inputType: 'number'},
        { value: '斤', placeholder: '单位(如: 斤)', inputType: 'unit', prop: 'unit'},
      ]
    },
    // 设置表格
    setItemValue() {
    ...
    // 同步规格单位
    unitSelectChange(value, i) {
      console.log(value,i)
      this.tableBody.map(label => {
        label.itemValue[i].value = value
        let row = label.itemValue[4]
        if (value === '斤') {
          if (row && row.prop == 'weight') {
            row.value = 0.5
            row.disabled = true
            this.tableTitle[4] && this.tableTitle[4].prop == 'weight' && (this.tableTitle[4].disabled = true)
          }
        } else {
          row.disabled = false
          this.tableTitle[4] && this.tableTitle[4].prop == 'weight' && (this.tableTitle[4].disabled = false)
        }
      })
      this.compareValue()
    },
    // 规格单位变更
    unitSelectChange2(value, i) {
      let item =  this.tableBody[i].itemValue[4]
      if (value === '斤') {
        if (item && item.prop == 'weight') {
          item.value = 0.5
          item.disabled = false
          this.tableTitle[4] && this.tableTitle[4].prop == 'weight' && (this.tableTitle[4].disabled = true)
        }
    ...
    setTable(productItems) {
      productItems.map((item, i) => {
        ...
        itemValue[2].value = item.supplierPrice / 100
        itemValue[3].value = item.weight / 1000
        itemValue[4].value = item.minOrderQuantity
        itemValue[5].value = item.qty
        itemValue[6].value = item.unit
        ...
        for (let key in item) {
          ...
          if (item.unit == '斤') {
            this.tableBody[i].itemValue[4] && this.tableBody[i].itemValue[4].prop == 'weight' && (this.tableBody[i].itemValue[4].disabled = true)
          } else {
            this.tableBody[i].itemValue[4] && this.tableBody[i].itemValue[4].prop == 'weight' && (this.tableBody[i].itemValue[4].disabled = false)
          }
        }
      })
        ...
        if (valid) {
          ...
          this.getProperty(model).then(
            model => {
              ...
              model.supplierPrice = parseFloat(model.supplierPrice) * 100
              model.weight = parseFloat(model.weight) * 1000
              if (model.isMultiSpec) {
                // 多规格走这里
                let maxPrice = model.price
                let minPrice = model.price
                let minUnit = model.unit
                ...
                model.productItems.map((item, i) => {
                  ...
                  item.supplierPrice = parseFloat(item.supplierPrice) * 100
                  item.weight = parseFloat(item.weight) * 1000
