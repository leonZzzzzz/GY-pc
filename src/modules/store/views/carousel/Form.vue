...
export default {
  ...
  methods: {
    ...
    getCarousel() {
      this.$http.getStoreCarousel({ id: this.id }).then(res => {
    ...
    success() {
      this.$refs.model.validate(valid => {
        if (valid) {
          ...
          this.loading = true
          this.saveModel(this.model.id ? 'updateStoreCarousel' : 'addStoreCarousel', model)
        }
      })
    },
    saveModel(type, model) {
      this.$http[type](model)
        .then(() => {
          this.loading = false
          this.$message({
            message: type === 'updateStoreCarousel' ? '修改成功' : '添加成功',
            type: 'success'
          })
          this.close(true)
        })
